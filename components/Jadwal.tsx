...
import { getFridayStandbyStatus, isFridayStandbyTable } from '../utils/fridayStandby';
import {
...
export const Jadwal: React.FC<JadwalProps> = ({
  ...
  const isFridayStandby = isFridayStandbyTable(activeTable.title);
  ...
  const handleSaveEntry = () => {
    ...
    if (!entryDate.trim() || (!isFridayStandby && (!entryStartTime || !entryEndTime))) {
      setFormError(isFridayStandby ? 'Mohon isi Tanggal.' : 'Mohon isi Tanggal dan pilih Jam Lembur.');
      return;
    }

    const { columnId, entryIndex } = activeEntryModal;

    const newEntryObj: OvertimeEntry = isFridayStandby
      ? {
          ...activeEntryModal.entry,
          id: activeEntryModal.entry?.id || `ent_${Date.now()}`,
          date: entryDate.trim(),
          purpose: activeEntryModal.entry?.purpose || '',
      ...
      <div className="bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 rounded-2xl shadow-xl overflow-hidden flex flex-col">
              ...
              {displayRows.map((_, rowIndex) => (
                ...
                >
                  ...
                  {activeTable.columns.map((col) => {
                    ...
                    const overtimeStatus = entry ? (isFridayStandby ? getFridayStandbyStatus(entry, statusClock) : getOvertimeStatus(entry, statusClock)) : undefined;
                    const isScheduled = overtimeStatus === 'scheduled';
                    const isOngoing = overtimeStatus === 'ongoing';
                          ...
                          <div
                            ...
                            className={`p-2.5 m-0.5 rounded-lg shadow-sm transition-all cursor-pointer text-center leading-snug group relative ${
                              isSelected
                                ...
                                : isScheduled
                                  ? 'bg-transparent text-slate-700 dark:text-slate-200 font-extrabold text-xs border border-dashed border-slate-400 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-98'
                                  : isOngoing
                                    ? 'bg-amber-100 dark:bg-amber-950/70 text-amber-950 dark:text-amber-100 font-extrabold text-xs border border-amber-500 dark:border-amber-400 ring-2 ring-amber-400/60 shadow-[0_0_16px_rgba(245,158,11,0.7)] hover:brightness-105 active:scale-98'
                            ...
                            /* Glow efek berjalan dipasang inline karena utility shadow
                               bawaan kartu menang urutan di stylesheet. */
                            style={isOngoing ? { boxShadow: '0 0 18px rgba(245, 158, 11, 0.85)' } : undefined}
                          >
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between text-xs text-slate-500 dark:text-slate-400 gap-2">
          <div className="flex items-center gap-3">
            ...
            {!isFridayStandby && (
              <span className="flex items-center gap-1.5 font-bold">
                <span className="w-3 h-3 rounded-full inline-block border-2 border-amber-500 bg-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.85)]" />
                Berjalan
              </span>
            )}
            <span className="flex items-center gap-1.5 font-bold">
          ...
          <div className="font-mono text-[11px] font-bold text-slate-500">
            ...
            Total {isFridayStandby ? 'Tanggal Tercatat' : 'Lembur Tercatat'}: <span className="text-amber-600 font-extrabold">{activeTable.columns.reduce((sum, c) => sum + c.entries.length, 0)}</span>
          </div>
        </div>
      </div>
            ...
            <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-slate-950 p-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-black tracking-wide uppercase flex items-center gap-2">
                  ...
                  {isFridayStandby
                    ? (activeEntryModal.entry ? 'Edit Tanggal Standby Jumat' : 'Catat Tanggal Standby Jumat')
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-extrabold text-slate-600 dark:text-slate-300 uppercase tracking-wider block">
                  {isFridayStandby ? 'Tanggal Standby Jumat (DD/MM/YYYY)' : 'Tanggal Lembur (DD/MM/YYYY)'}
                </label>
                <input
                  type="text"
                  value={entryDate}
                ...
                <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {(activeEntryModal.entryIndex !== undefined || activeEntryModal.entry !== undefined) ? 'Simpan Perubahan' : isFridayStandby ? 'Simpan Tanggal' : 'Simpan Lembur'}
                    </button>
                  </div>
                </div>
              )}
